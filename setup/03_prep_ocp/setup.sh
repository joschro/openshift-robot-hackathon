...
PRJ_SUFFIX='robo-hackathon-infra'
GITHUB_ACCOUNT=${GITHUB_ACCOUNT:-nexus-Six}
GITHUB_REF=${GITHUB_REF:-ocp-3.11}

function deploy() {
  oc $ARG_OC_OPS new-project hub-controller-$PRJ_SUFFIX   --display-name="Hub Controller Robo Hackathon Infra"  
  oc $ARG_OC_OPS new-project dev-$PRJ_SUFFIX   --display-name="Dev Robo Hackathon Infra"
  oc $ARG_OC_OPS new-project stage-$PRJ_SUFFIX --display-name="Stage Robo Hackathon Infra"
  oc $ARG_OC_OPS new-project cicd-$PRJ_SUFFIX  --display-name="CI/CD Robo Hackathon Infra"
...
echo_header "OpenShift Robo Hackathon Environment ($(date))"

case "$ARG_COMMAND" in
    delete)
        echo "Deleting environment..."
        oc $ARG_OC_OPS delete project dev-$PRJ_SUFFIX stage-$PRJ_SUFFIX cicd-$PRJ_SUFFIX hub-controller-$PRJ_SUFFIX
        echo
        echo "Delete completed successfully!"
        ;;
      
    idle)
        echo "Idling environment..."
        make_idle
        echo
